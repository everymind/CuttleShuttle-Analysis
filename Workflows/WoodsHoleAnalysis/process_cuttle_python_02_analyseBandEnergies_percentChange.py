...
boxplots_of_TSP_onset('ProcessCuttlePython', 'PercentChange', 'power at frequency', 'all', first_exits_3sigCI, first_exits_y_scatter, median_first_exits['all'], N_freqBands_to_plot, baseline_frames, TGB_bucket_raw, today_dateTime, plots_folder)
boxplots_of_TSP_onset('ProcessCuttlePython', 'PercentChange', 'power at frequency', 'catches', first_exits_3sigCI, first_exits_y_scatter, median_first_exits['catches'], N_freqBands_to_plot, baseline_frames, TGB_bucket_raw, today_dateTime, plots_folder)
boxplots_of_TSP_onset('ProcessCuttlePython', 'PercentChange', 'power at frequency', 'misses', first_exits_3sigCI, first_exits_y_scatter, median_first_exits['misses'], N_freqBands_to_plot, baseline_frames, TGB_bucket_raw, today_dateTime, plots_folder)

def boxplots_of_TSP_onset(analysis_type_str, preprocess_str, metric_str, ts_category_str, first_exits_list, y_scatter_list, median_exits_list, N_fb_to_plot, baseline_len, TGB_bucket, todays_dt, plots_dir):
    N_TS_str = 'Number of tentacle shots: '
    for f, freq_band in enumerate(range(N_fb_to_plot)):
        N_TS_thisFB = len(first_exits_list[ts_category_str][freq_band])
        if f == N_fb_to_plot:
    ...
    for i, m_exit in enumerate(median_exits_list):
        if i == len(median_exits_list):
