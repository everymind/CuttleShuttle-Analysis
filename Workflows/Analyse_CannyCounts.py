...
for rep in range(No_of_repetitions):
    # shuffle test
    ...
    global005sig_ZSess_ErrOfSigTB[rep] = {'upper bound': None, 'lower bound': None}
    for timebin in range(360):
        # collect all edge scores for each time bin
    ...
    UB_corrected_ErrOfSigTB[rep] = 99.9
    LB_corrected_ErrOfSigTB[rep] = 0.1
    ...
    while N_violations_ZSess_UBcorrected_ErrOfSigTB>50 or N_violations_ZSess_LBcorrected_ErrOfSigTB>50:
        global005sig_ZSess_ErrOfSigTB[rep]['upper bound'], global005sig_ZSess_ErrOfSigTB[rep]['lower bound'] = find_bounds_for_sig(ZSessEdgeScores_byTB_ErrOfSigTB[rep], UB_corrected_ErrOfSigTB[rep], LB_corrected_ErrOfSigTB[rep])
        # check how many of these random traces violate the p<0.05 generated by timebin-wise shuffle test
        N_violations_ZSess_UBcorrected_ErrOfSigTB, N_violations_ZSess_LBcorrected_ErrOfSigTB = check_violations_sigBounds(shuffMeans_ZSess_traces_ErrOfSigTB[rep], global005sig_ZSess_ErrOfSigTB[rep]['upper bound'], global005sig_ZSess_ErrOfSigTB[rep]['lower bound'])
    ...
    # find where observed data crosses corrected bounds for first time
    print('Finding first timebin when difference of catch versus miss edge counts are significant, rep = {r}...'.format(r=rep))
    for tb in range(len(Observed_DiffMeans_ZSess)):
